...
export const QuestionsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        setState((prev) => ({
          ...
          sheets,
          questions,
          activeSheet: prev.activeSheet || (sheets.length > 0 ? sheets[0].id : null), // Only set if `activeSheet` is null
          loading: false,
        }));
      } catch (error: any) {
        console.error("Error fetching data:", error);
  ...
  useEffect(() => {
    if (state.activeSheet === null && state.sheets.length > 0) {
      setState((prev) => ({
        ...prev,
        activeSheet: state.sheets[0].id, // Only set if `activeSheet` is null
      }));
    }
